...
async def set_currency(user_id: int, currency_data: CurrencyChangeSchema,
                       session: AsyncSession = async_session_maker()):
    subquery = select([Wallet.id]).where(Wallet.user_id == user_id).scalar_subquery()

    stmt = update(Currency).values(**currency_data.dict().pop("name")).where(
        (Currency.wallet_id == subquery) & (Currency.name == currency_data.name)
    )
    await session.execute(stmt)
    await session.commit()
    await session.close()
...
async def set_balance(user_id: int, currency_data: CurrencyChangeSchema, session: AsyncSession = async_session_maker()):
    ...
    stmt = update(Currency).values(**currency_data.dict().pop("name")).where(
        (Currency.wallet_id == subquery) & (Currency.name == "USD")
    )
    await session.execute(stmt)
    await session.commit()
    await session.close()
...
async def make_transaction(user_id: int, transaction_data: TransactionCreateSchema,
                           session: AsyncSession = async_session_maker()):
    balance = await session.execute(
        select([Currency.quantity]).where(and_(Wallet.user_id == user_id, Wallet.currency.name == "USD")))
    ...
    if bought_price:
        if balance.scalar() < bought_price:
            raise ValueError({"error": "Your balance is less than the crypto currency price"})
        balance = balance.scalar() - bought_price
        currency = await session.execute(select(Currency).where(Currency.name == transaction_data.currency))

        if not currency:
            wallet_id = await session.execute(select([Wallet.id]).where(Wallet.user_id == user_id))
            new_currency_data = {
    ...
    if sold_price:
        ...
        if not currency_quantity or currency_quantity.scalar() <= 0:
            raise ValueError({"error": f"You have no {transaction_data.currency} coins"})
